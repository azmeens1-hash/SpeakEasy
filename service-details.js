(function () {
  ...
  function setRTL(rtl) {
    ...
    if (btn) { btn.title = rtl ? 'Switch to LTR' : 'Switch to RTL'; btn.innerHTML = '<i class="fas fa-exchange-alt"></i>'; }
  ...
  if (rtlToggle) { rtlToggle.addEventListener('click', function(e) { e.stopPropagation(); setRTL(!isRTL()); }); }
  ...
  if (hamburger && navLinks) {
    ...
    document.addEventListener('keydown', function(e) {
      if (e.key === 'Escape') { navLinks.classList.remove('open'); hamburger.classList.remove('open'); document.body.style.overflow = ''; }
    });
    document.addEventListener('click', function(e) {
  ...
  document.querySelectorAll('.dropdown > a').forEach(function(link) {
    link.addEventListener('click', function(e) {
      if (navLinks && navLinks.classList.contains('open')) { e.preventDefault(); link.closest('.dropdown').classList.toggle('open'); }
    });
  });
  ...
  window.switchTab = function(btn, id) {
    ...
    document.querySelectorAll('.tab-content').forEach(function(t) { t.style.display = 'none'; t.classList.remove('show'); });
    var tc = document.getElementById(id);
    if (tc) { tc.style.display = 'flex'; tc.classList.add('show'); }
  };

  window.toggleModule = function(header) { header.closest('.module-item').classList.toggle('open'); };
})();