(function () {
  ...
  /* ── Theme — icon-swap treatment (Font Awesome fa-sun / fa-moon) ── */
  ...
  if (themeToggle) {
    themeToggle.addEventListener('click', function (e) {
      ...
      try { localStorage.setItem(THEME_KEY, isDark ? 'dark' : 'light'); } catch (e2) {}
    });
  }

  /* ── RTL — circular button + text-swap treatment ── */
  ...
  if (rtlToggle) {
    ...
    rtlToggle.title = isRtlInit ? 'Switch to LTR' : 'Switch to RTL';
    ...
    rtlToggle.addEventListener('click', function (e) {
      ...
      try { localStorage.setItem(RTL_KEY, newDir); } catch (e2) {}
    });
  }
